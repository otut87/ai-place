...
describe('runPublicDiagnosticAction', () => {
  ...
  it('정상 → scanSite 결과 그대로 반환', async () => {
    mockScanSite.mockResolvedValue({ url: 'https://x', fetchedAt: 't', score: 80, checks: [] })
    const { runPublicDiagnosticAction } = await import('@/lib/actions/diagnose')
    const r = await runPublicDiagnosticAction('https://x.com')
    expect(r.score).toBe(80)
    expect(mockScanSite).toHaveBeenCalledWith('https://x.com')
  })
})
