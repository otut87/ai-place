...
| **Phase 7** | Admin 재설계 (IA·성능·검수·결제·자동화·분석) | H-01 + T-058 ~ T-084 + T-CP1/CP2 | 3개월 |
...
## Phase 7 — Admin 재설계 (2026-04-18 등록)

> 소스: `docs/admin 구현 계획서.md`, `docs/admin 설계안.md`, `docs/admin 속도 진단.md`, `docs/리뷰/추가리뷰.md`.
> 원칙: 기존 테이블(`ai_generations`/`place_audit_log`/`citation_results`/`blog_posts` 등)에 컬럼을 alter 로 추가하고 **같은 개념에 컬럼 산포 금지**.

### Milestone 0 — Hotfix

## H-01. `citation_results` 코드-DB 정합화 🔜 [GEO]

**근거**: `001_initial_schema.sql` 이 먼저 `citation_results` 를 정의 (prompt_id uuid FK, cited_sources/places text[], session_id NOT NULL). Phase 6 의 `016_citation_results.sql` 은 `create table if not exists` 라 실질 noop. 코드는 jsonb·nullable 가정으로 작성되어 실환경 insert 시 타입 에러.
...
- [ ] `016_citation_results.sql` 삭제 또는 001 보조 인덱스 추가만 남김 (`idx_citations_tested_at desc`, `idx_citations_aiplace_cited partial`)
...
- [ ] `AdminNavLink` 공용 컴포넌트 추출 (prefetch 기본값 false)
- [ ] 스냅샷 또는 `toHaveAttribute('prefetch', 'false')` 테스트
- **DoD**: admin 이동 시 `_rsc` prefetch 호출 0건

## T-059. Production 빌드 검증 🔜 [Ops]
**근거**: 속도 진단 원인 2 — 청크 11개 + turbopack 의심
- [ ] Vercel build logs 에서 `next build` + `NODE_ENV=production` 확인
- [ ] `package.json` `dev/build` 스크립트에 `--turbo` 플래그 없는지 재확인
- [ ] `/admin/*` 에 `export const runtime = 'nodejs'` 명시
- [ ] bundle analyzer 실행 후 admin 청크 리포트 첨부
- **DoD**: 프로덕션 빌드 청크 목록에 turbopack 흔적 없음

## T-060. GA / 외부 스크립트 `lazyOnload` 🔜 [Ops]
**근거**: 속도 진단 §그외 — GA 503 + 메인 번들 점유. `analytics.tsx:38` = `strategy="afterInteractive"`
- [ ] `<Script strategy>` 를 `lazyOnload` 로 변경
- [ ] 어드민 레이아웃(`/admin/*`, `/owner/*`) 에서 Analytics 컴포넌트 제외
- **DoD**: `/admin/places` 로드 시 GA 스크립트가 DOMContentLoaded 이후에만 로드

### Milestone 2 — 검수 큐 중심 IA 재구성

## T-061. 좌측 사이드바 + 글로벌 바 🔜 [Admin]
**근거**: 설계안 §2 IA
- [ ] `src/components/admin/sidebar.tsx` — 아이콘+라벨+접기 (localStorage persist)
- [ ] `src/components/admin/topbar.tsx` — ⌘K 자리·검수 뱃지·결제 실패 뱃지·알림·프로필
- [ ] `src/app/admin/layout.tsx` 리팩터 — 모든 admin 페이지 동일 섀시
- [ ] shadcn `--primary` 를 Place Green(#00a67c) 으로 오버라이드
- **DoD**: 모든 admin 페이지 동일 섀시, 본문 풀폭(max-width 제거)

## T-062. `/admin/review` 검수 큐 v1 🔜 [Admin]
**근거**: 설계안 §3.2 — "가장 자주 쓰는 페이지"
- [ ] 좌측 pending 큐 리스트, 우측 분할 뷰(AI 원본 ↔ 최종본 diff)
- [ ] 단축키: `⌘↵` 승인, `⌘⌫` 반려, `J/K` 네비
- [ ] 반려 사유 태그(`fact_error | tone | seo | duplicate`)
- [ ] 서버 액션 `approvePlace` / `rejectPlace` — 기존 `bulk-places.ts` status 변경 + audit 로깅 재사용
- **DoD**: pending 업체를 키보드만으로 처리 가능, 사유 집계 가능

## T-063. 저장 토스트 + 파괴 액션 모달 🔜 [Admin]
**근거**: 설계안 §4, 현재 `window.confirm()` 2곳
- [ ] `useToast` hook 또는 `sonner` 도입
- [ ] `<ConfirmNameModal name={업체명}>` — 삭제·대량 비활성
- [ ] `places-table.tsx`, `place-actions.tsx` 의 `window.confirm()` 교체
- **DoD**: 파괴 액션 실행 시 업체명 타이핑 일치 강제

### Milestone 2 — 대시보드·목록·상세

## T-064. `/admin` 운영 대시보드 🔜 [Admin]
**근거**: 설계안 §3.1, 현재 2-link 런처
- [ ] 상단 4 액션 카드: 검수 대기 / 오늘 발행 예정 / 실패 작업 / 결제 실패·만료임박
- [ ] 중단 지표: AI 크롤러 추이·MRR·결제 실패
- [ ] 하단 활동 로그(`place_audit_log` + T-068 actor_type 아이콘)
- **DoD**: "오늘 해야 할 일" 한 장 파악

## T-065. Places 목록 보강 (증분) 🔜 [Admin]
**근거**: T-046 위에 증분. 필터·URL 쿼리·일괄 액션은 이미 완료.
- [ ] `places-table.tsx` 에 컬럼 추가: 구독상태(T-070 조인) · AI 노출 점수(`quality_score`)
- [ ] 필터에 `subscription` · `min_quality_score` 추가
- **DoD**: `?subscription=paid&min_quality_score=70` 동작

## T-066. `/admin/places/[id]` 탭 구조 + AI 노출 체크리스트 🔜 [Admin]
**근거**: 설계안 §3.4, 현재 145줄 flat form
- [ ] 탭: 개요/서비스/FAQ/태그/블로그/SEO/자동화 이력/변경 로그
- [ ] 자동저장 + 토스트 + diff 보기 (`recordUpdateDiffs` 재사용)
- [ ] 우측 사이드바 체크리스트 — `place-validation.ts` 완성도 score 재활용
- **DoD**: 완성도 90점 미만 업체는 공개 노출 경고 배너

## T-067. SC 병렬화 + `cache()` 적용 🔜 [Ops]
**근거**: 속도 진단 원인 4 — DOM Ready 1.8s
- [ ] `/admin/places/page.tsx` places query 도 `Promise.all`
- [ ] 인증 체크 미들웨어 단일화 (admin 내부 `requireAuth()` 중복 제거)
- [ ] `React.cache()` 로 `getCities`/`getCategories` 메모
- [ ] `loading.tsx` Suspense 경계
- **DoD**: `/admin/places` DOM Ready < 600ms

## T-068. `place_audit_log.actor_type` 컬럼 🔜 [Ops]
**근거**: 설계안 §4, T-055 누락분
...
- [ ] 변경 로그 UI 에 사람/로봇 아이콘
- **DoD**: 감사 로그에서 자동/수동 구분 가시화

## T-069. `places.field_meta` JSONB (필드별 출처·신뢰도) 🔜 [GEO][Admin]
**근거**: 설계안 §5. **중복 방지 — 컬럼 1개로 통합**
...
- [ ] `src/lib/admin/field-meta.ts` 검증/머지 라이브러리 (TDD)
- [ ] T-066 탭에 칩 렌더
- **DoD**: AI 생성 필드는 "AI·0.82" 뱃지, 수동은 "수동"

### Milestone 3 — 결제·구독 (MRR 이탈 방어)

## T-070. 결제 스키마 + 빌링키 테이블 🔜 [Billing]
...
## T-071. PG 어댑터 + 포트원 1순위 🔜 [Billing]
**근거**: 설계안 §3.10
- [ ] `src/lib/billing/adapter.ts` — `issueBillingKey / chargeOnce / scheduleNext / revoke`
- [ ] `src/lib/billing/portone.ts` 1차 구현 (토스페이먼츠 호환)
- [ ] 환경변수 없으면 mock 어댑터 폴백 (notify 패턴 준용)
- [ ] 테스트: 성공/실패/만료/한도초과 4경로
- **DoD**: 실 PG 키 없이도 개발·테스트 완결

## T-072. 결제 정책 문구 단일 소스 🔜 [Billing]
**근거**: 설계안 §3.11
- [ ] `src/lib/billing/policy.ts` 상수
- [ ] 렌더: 결제 화면 / 이용약관 / 영수증 / FAQ 4곳
- [ ] `notify/email` 영수증 템플릿 삽입
- **DoD**: 4곳 동일 상수 import

## T-073. `/admin/billing/failures` 결제 실패 큐 🔜 [Billing]
- [ ] 실패 컬럼 + PG 응답코드 한국어 매핑 + 재시도 스케줄(+1d/+3d/+7d)
- [ ] 3회 실패 → `suspended`
- [ ] 알림: notify/* 재사용, 이벤트 `payment.failed`, `payment.retry_exhausted`
- **DoD**: 실패 1건 4영업일 이내 재시도 3회

## T-074. 카드 만료임박 자동 안내 🔜 [Billing]
- [ ] Vercel Cron — 매일 `billing_keys` 스캔
- [ ] 30일/7일 이내 만료 고객에 이메일, 이벤트 `billing.expiry_warning`
- [ ] 대시보드(T-064)에 발송 이력 카운트
- **DoD**: 만료 30일 전/7일 전 각 1회 발송

### Milestone 4 — 자동화·콘텐츠

## T-075. Regenerate 공통 컴포넌트 🔜 [GEO][Admin]
**근거**: 설계안 §5, 현재 `CandidatePicker` 는 register 전용
- [ ] `<RegenerateButton field options={tone,length,keywords}>`
- [ ] T-052 `generateContentCandidates` 재사용 (단일/다중 후보)
...
- [ ] UI: 실패 필터 + 재시도 + 페이로드 확인
- [ ] API 쿼터 표시 (네이버·카카오·Google Places)
- [ ] LLM 토큰·비용 집계는 **기존 `ai_generations`(013) 재사용** — 신규 테이블 금지
- **DoD**: 파이프라인 실패 가시화 + 재시도 원클릭

## T-077. 프롬프트 템플릿 버전 관리 🔜 [GEO]
**근거**: 설계안 §3.11
- [ ] `022_prompt_templates.sql` — `(id, category, version, system_prompt, user_template, created_at, active)`
- [ ] 서버 액션 list/upsert/activate
- [ ] A/B 집계 — **기존 `ai_generations` 에 `prompt_template_id` 컬럼 1개만 추가**
- **DoD**: 카테고리별 버전 저장·활성화·통과율 비교

## T-078. 블로그 캘린더 + 검수 큐 🔜 [SEO][GEO]
**근거**: 설계안 §3.6
- [ ] `/admin/blog` 월/주 캘린더
- [ ] 토픽 큐 — `blog_posts`(004) `status` 재사용
- [ ] 편집기: 마크다운 + 프리뷰 + 내부링크 추천
...
## T-080. 수동 등록 → 자동 파이프라인 트리거 🔜 [Admin]
**근거**: 설계안 §3.5
- [ ] `/admin/register` 에 "AI 자동완성 등록" 버튼 → `pipeline_jobs` insert (T-076 테이블 재사용)
- [ ] 수집 → 생성 → 검수 통합 테스트
- **DoD**: 업체명 한 줄 입력으로 검수 큐 pending 진입

### Milestone 5 — 분석·고도화

## T-081. `/admin/seo` AI 봇 로그 🔜 [GEO]
**근거**: 설계안 §3.8 + 추가리뷰 §10
- [ ] 미들웨어에서 AI 봇 User-Agent 감지 → `bot_visits` insert
- [ ] GPTBot/ClaudeBot/PerplexityBot/CCBot/Google-Extended 10+ 종
- [ ] 봇별 추이·hit URL·404 비율 시각화
...
- [ ] `cmdk` 도입
- [ ] 업체 검색 / 페이지 이동 / 최근 본 항목 / 검수 큐 이동
- **DoD**: 어드민 전역에서 ⌘K 업체 검색

## T-084. A/B 프롬프트 실험 + 품질 지표 🔜 [GEO]
- [ ] T-077 프롬프트 버전 ↔ T-027 `quality_score` 연결
- [ ] 버전별 평균 스코어 + 검수 통과율 비교 UI
- **DoD**: "v3 프롬프트가 +12% 통과율" 관측 가능
...
- [ ] 업체 상세 하단 참고 문구 강제 (T-004 재사용)

## T-CP2. 리뷰 저작권 안전 🔜 [Compliance]
**근거**: 추가리뷰 §4 리스크 3
- [ ] Google/Naver 리뷰 본문 스크래핑 금지 — URL + 점수만 보관
- [ ] `review_summaries.sampleQuote` 를 패러프레이즈 여부 플래그로 재정의
- [ ] T-021~T-023 요약 파이프라인에 인용 길이 50자 제한

---

### Phase 7 스키마 중복 방지 원칙
- 같은 개념(AI 생성 메타 등)은 **1 테이블 JSONB 1 컬럼** 으로 통합
- `ai_generations`(013) / `place_audit_log`(015) / `citation_results`(001) / `blog_posts`(004) 등 기존 테이블에 **alter table** 로 증분
- 유사 이름의 새 테이블 금지 — 조인 키 먼저 확인
- 알림·로깅은 Phase 6 의 `notify/*`, `audit/*` 인프라만 확장
